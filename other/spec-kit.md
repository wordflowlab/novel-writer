...
```
1. /constitution  →  项目宪法(最高原则,定义项目DNA)
2. /specify       →  功能规格(单一真相源,定义做什么和为什么)
3. /clarify       →  澄清决策(消除歧义,标记待确认点)
4. /plan          →  技术方案(定义怎么做,选择技术栈)
5. /tasks         →  任务分解(可执行步骤,标注并行性)
6. /analyze       →  质量门(一致性验证,CRITICAL问题必修)
7. /implement     →  执行实现(TDD方式生成代码)
```
...
| 命令 | 输入 | 输出 | 关键约束 |
|------|------|------|---------|
| `/constitution` | 项目理念 | `项目宪法.md` | 定义不可妥协的原则 |
| `/specify` | 功能描述 | `功能规格.md` | 只写做什么/为什么,不写怎么做 |
| `/clarify` | 规格文档 | 更新后的规格 | 强制解决所有待确认点 |
| `/plan` | 规格+技术选型 | `技术方案.md`+`数据模型.md` | 必须通过宪法检查 |
| `/tasks` | 技术方案 | `任务列表.md` | 标注依赖和并行性 |
| `/analyze` | 规格+方案+任务 | 分析报告 | **只读**,不修改文件 |
| `/implement` | 任务列表 | 代码+测试 | 严格TDD: 先测试后实现 |
...
```bash
/constitution 创建聚焦代码质量、测试标准、教育行业合规性和性能要求的项目原则
```
...
宪法是项目的"基因",定义了**不可妥协的原则**。后续所有`/plan`生成的方案都必须通过这些检查,否则`/analyze`会报CRITICAL错误,阻止实现。
...
```bash
/specify 开发"作业批改"功能。教师可以发布作业(题目+截止时间+附件),学生提交作业(支持上传图片/PDF),教师在线批改并给分(0-100分+文字评语)。初始版本: 3个教师、30个学生、2个班级。作业状态包括: 未提交、已提交、已批改、需修改。学生可查看批改结果和历史记录,教师可统计班级提交率和平均分。逾期提交的作业标记为红色。权限控制: 学生只能看到自己的作业,教师只能看到自己任教班级的作业。
```
...
```bash
/plan 使用Spring Boot 3.2 + MyBatis-Plus + MySQL 8.0 + Redis作为缓存。文件上传使用阿里云OSS,统计数据用Redis缓存5分钟。前端Vue3 + Element Plus,图片预览用viewerjs。
```
...
### 修复后:
重新运行`/analyze`确认CRITICAL=0,然后执行`/implement`。
```
...
这是`/plan`层级的问题(数据模型设计),需回退到`/plan`
...
| 变更类型 | 影响层级 | 需要回退到 |
|---------|---------|-----------|
| 修改按钮颜色 | 实现细节 | `/implement` |
| 添加"is_excellent"字段 | 数据模型 | `/plan` |
| 新增"展示功能" | 功能定义 | `/specify` |
| 改变项目架构 | 项目DNA | `/constitution` |

**当前变更**: 新增功能 → 回退到 `/specify`
...
| 条款 | 内容 | 如何检测 | 违规后果 |
|------|------|---------|---------|
| **第一条: 模块优先** | 功能先做成独立Service | `/analyze`检测是否直接在Controller写业务逻辑 | CRITICAL错误 |
| **第二条: 接口清晰** | 统一Result<T>返回格式 | `/analyze`检测Controller返回值类型 | HIGH警告 |
| **第三条: 测试先行** | 先写测试后实现 | `/implement`检查任务顺序 | 拒绝生成代码 |
| **第四条: 数据安全** | 敏感信息加密存储 | `/analyze`检测是否有`_encrypted`字段 | CRITICAL错误 |
| **第七条: 简单性** | ≤3个Maven模块 | `/plan`生成时检查 | 需书面justification |
| **第八条: 反过度抽象** | 直接用MyBatis-Plus | `/analyze`检测是否有多余的Wrapper | 需justification |
| **第九条: 集成测试** | 用真实MySQL测试 | `/implement`检查测试配置 | 测试套件不通过 |
...
| 级别 | 定义 | 典型问题 | 如何处理 |
|------|------|---------|---------|
| **CRITICAL** | 违反宪法MUST条款,或核心功能零覆盖 | 测试先行未遵守、数据安全违规、主功能无任务 | **阻塞 `/implement`**,必须修复 |
...
**场景**: 运行`/implement`后发现AI生成的批改页面没有"批量批改"功能,现在想加。
...
"批量批改"是新功能需求 → 影响`/specify`层级

**步骤2: 回退到 `/specify`,更新spec.md**
...
| 变更类型 | 回退到 | 示例 |
|---------|--------|------|
| UI样式 | `/implement` | 按钮颜色改蓝色 |
| 添加字段 | `/plan` | 作业表加difficulty字段 |
| 新增功能 | `/specify` | 添加批量批改 |
| 改核心逻辑 | `/specify` | 批改改用AI自动评分 |
| 架构变更 | `/constitution` | 改为微服务架构 |
...
CRITICAL = **阻塞`/implement`的问题**,必须修复,否则:
- 违反项目宪法(不可妥协原则)
- 或导致核心功能无法实现
...
2. **增量更新**: `/implement`只重新生成受影响的部分
3. **自动一致性**: `/analyze`确保规格+方案+任务+代码一致
...
**强制运行**:
1. **After `/tasks`, before `/implement`**(最重要!)
2. **需求变更后** - 更新spec.md后运行
3. **Code review前** - 确保规格与代码一致

**可选运行**:
- After `/plan`: 检查方案是否覆盖所有需求
- 每周定期: 检查规格漂移
