...
- **templates/commands/analyze.md**: 完全重写,详细说明两种分析模式
...
#### 1. specification.md 增强 (templates/commands/specify.md)
...
#### 2. creative-plan.md 增强 (templates/commands/plan.md)
章节段表格增加"活跃线索"和"交汇点"列:
- 标注每个章节段推进哪些线索
- ⭐⭐⭐ 主推进 / ⭐⭐ 辅助 / ⭐ 背景
- 明确交汇点所在章节

#### 3. tasks.md 增强 (templates/commands/tasks.md)
每个写作任务增加线索相关字段:
- **涉及线索**: 本章推进哪些线索及优先级
- **交汇点**: 本章是否为交汇点
- **伏笔埋设/揭晓**: 本章涉及的伏笔操作

#### 4. plot-tracker.json 增强 (templates/commands/track-init.md)
...
### 🐛 Bug 修复
- **命令模板缺失**：补全 `.specify/templates/commands/` 中的七步方法论命令模板
  - 添加 `constitution.md` - 创作宪法命令
  - 添加 `specify.md` - 故事规格命令
  - 添加 `plan.md` - 创作计划命令
  - 添加 `tasks.md` - 任务分解命令
  - 添加 `analyze.md` - 综合验证命令
