...
8. **`/method`** - 智能选择最适合的写作方法
9. **`/style`** - 设定创作风格和准则
10. **`/story`** - 创建故事大纲（智能选择方法模板）
11. **`/outline`** - 规划章节结构（应用所选方法）
12. **`/chapters`** - 生成具体的写作任务

#### 追踪管理命令

13. **`/plot-check`** - 检查情节发展的一致性
14. **`/timeline`** - 管理和验证故事时间线
15. **`/relations`** - 追踪角色关系变化
16. **`/world-check`** - 验证世界观设定一致性
17. **`/track`** - 综合追踪创作进度和内容

#### 专家模式命令（需要 --with-experts）

18. **`/expert`** - 列出可用专家
19. **`/expert plot`** - 剧情结构专家
20. **`/expert character`** - 人物塑造专家
21. **`/expert world`** - 世界观设计专家
22. **`/expert style`** - 文风语言专家
